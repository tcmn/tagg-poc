...
public class TaggCompress {
	...
	public static void main(String[] args) {
		...
		try {
			while (true) {
				 ...
				 // 10 segundos
				 // 5 trades
				 // Logica do tempo e tamanho do bloco
		         for (ConsumerRecord<String, String> record : records) {
		             System.out.printf("offset = %d, key = %s, value = %s%n", record.offset(), record.key(), record.value());
		             String aggregatedTrade = aggregateTrade(record.value());
	            	 producer.send(new ProducerRecord<String, String>(Topics.TRADES_AGGREGATEDS, aggregatedTrade));
		         }
			}
		} catch (Throwable e) {
	...
	private static String aggregateTrade(String value) {
		return null;
	}
